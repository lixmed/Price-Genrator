...
def generate_pdf_from_data(items, total, company_details, hdr_path="q2.png", ftr_path="footer (1).png"):
    """Generate a PDF from quotation data."""
...
if not st.session_state.history:
    ...
    # if st.button("👉 Go to Quotation Builder"):
    #     st.switch_page("app.py")
else:
            ...
            with col1:
                if st.button(f"📄 Regenerate PDF", key=f"regen_{idx}_{quote['hash']}"):
                    with st.spinner("Rebuilding PDF..."):
                        pdf_file = generate_pdf_from_data(quote["items"], quote["total"], st.session_state.company_details)
                        if pdf_file:
            ...
            with col2:
                if st.button("🗑️ Delete", key=f"del_{idx}_{quote['hash']}"):
                    ...
                    if st.session_state.get(f"confirm_delete_{idx}"):
                        # Actually delete
                        st.session_state.history.pop(len(st.session_state.history) - 1 - idx)  # reverse index
                        st.success("🗑️ Quotation deleted!")
                        st.rerun()
                    else:
                        # Ask for confirmation
                        st.session_state[f"confirm_delete_{idx}"] = True
                        st.warning(f"⚠️ Press 'Delete' again to confirm.")
                        st.rerun()